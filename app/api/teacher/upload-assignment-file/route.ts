...
export async function POST(request: Request) {
  try {
    ...
    const storagePath = `assignments/${user.id}/${timestamp}-${randomString}.${extension}`
    ...
    console.error('Error in upload-assignment-file API:', error)
    return NextResponse.json(
...
export async function DELETE(request: Request) {
  try {
    ...
    console.error('Error in delete assignment file API:', error)
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Internal server error' },
      { status: 500 }
    )
